...
Public Sub Initialize As PDFMaker
	...
	BANano.DependsOnAsset("html2pdfmake.js")
...
'create a watermark
Sub CreateWaterMark(Text As String, color As String, opacity As Double, bold As Boolean, italics As Boolean, angle As Int, fontSize As Int) As PDFText
	Dim wm As PDFText = CreateText(Text)
	wm.style.color = color
	wm.style.opacity = opacity
	wm.style.bold = bold
	wm.style.italics = italics
	wm.style.angle = angle
	wm.style.fontsize = fontSize
	Return wm
End Sub
...
'add html from layout
Sub AddHTML(sourceID As String) As PDFMaker
	sourceID = sourceID.tolowercase
	sourceID = sourceID.Replace("#","")
	If sourceID = "" Then Return Me
	'get the element
	Dim el As BANanoElement = BANano.GetElement($"#${sourceID}"$)
